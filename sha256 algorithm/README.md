...
## Windows quick start (install Ruby + run with relative paths)

If you’re on Windows and new to Ruby, here’s the quickest way to get running from the repository root.

1) Install Ruby

- Download “Ruby+Devkit” from https://rubyinstaller.org (3.x is fine).
...
If Ruby isn’t found, add it to PATH for this session (adjust the path for your version):
...
2) Run from the repository root using a relative path

From the root folder that contains the `sha256-animation` directory:

```powershell
ruby .\sha256-animation\sha256.rb abc
ruby .\sha256-animation\sha256.rb abc fast
ruby .\sha256-animation\sha256.rb 0b01100001
ruby .\sha256-animation\sha256.rb 0xaabbccdd
ruby .\sha256-animation\sha256.rb .\file.txt
```

Double-SHA256 (Bitcoin-style, expects hex):

```powershell
ruby .\sha256-animation\hash256.rb 0100000000...
```

3) Alternative: call Ruby by absolute path

If PATH isn’t set yet, you can still run:

```powershell
"C:\\Ruby34-x64\\bin\\ruby.exe" .\sha256-animation\sha256.rb abc fast
```

4) Terminal clearing

The animations call `system "clear"`. Running from the repo root (with the relative paths above) or inside the `sha256-animation` folder ensures the helper is found so screens clear properly. An external PowerShell window often renders the animation more cleanly than an integrated terminal.
